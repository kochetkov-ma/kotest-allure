...
internal class AllureMetadata(
    ...
    val allLinks: List<Link> = (issues()
        ...
        .distinct()
        .toList()
    val allLabels: List<Label> = listOf(epic, feature, severity, story, owner)
        .asSequence()
        .plus(jiraLabel())
        .plus(jiraLabelsFromName())
        .plus(jiraLabels())
        .plus(allureIdLabel())
        .plus(allureIdsFromTestName())
        .plus(tagLabel())
        .plus(tagLabels())
        .filterNotNull()
    ...
    private fun allureIdsFromTestName(): Collection<Label> {
        if (description == null) return emptyList()
        return ALLURE_ID.PATTERN.findAll(description.path().value).mapNotNull { result ->
            if (result.groups.size >= 2)
                result.groups[1]?.value?.takeIf { it.isNotBlank() }?.let { key ->
